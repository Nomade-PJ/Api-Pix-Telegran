// api/painel.js — serve o HTML do painel (HTML embutido)
...
<title>Painel — VIPs da Val</title>
...
<aside class="sidebar" id="sidebar">
  <div class="sidebar-header">
    <div class="sidebar-logo"><span class="icon">🔥</span>VIPs da <span>Val</span></div>
  ...
  <nav class="nav">
    ...
    <div class="nav-item" data-section="stats" onclick="navigate('stats', this)">
      <span class="ni">📈</span> Estatísticas
    </div>
  </nav>
...
<div class="main">
  ...
  <div class="content">
    ...
    <div id="section-dashboard" class="section active">
      ...
      <div class="dash-grid">
        <div class="dash-card">
          <div class="dash-card-title">⏱️ Últimas Transações</div>
          <div id="recentTx"></div>
        </div>
        <div class="dash-card">
          <div class="dash-card-title">👤 Novos Usuários</div>
    ...
    <div id="section-transactions" class="section">
      <div class="filters">
        <input class="filter-input" id="txSearch" placeholder="🔍 Buscar TXID ou Telegram ID..." oninput="loadTransactions()">
        ...
        <button class="btn btn-ghost btn-sm" onclick="loadTransactions()">🔄 Atualizar</button>
      </div>
    ...
    <div id="section-failures" class="section">
      <div class="section-header">
        <div class="section-title">⚠️ Falhas de Entrega</div>
    ...
    <div id="section-products" class="section">
      <div class="section-header">
        <div class="section-title">🛍️ Produtos</div>
    ...
    <div id="section-groups" class="section">
      <div class="section-header">
        <div class="section-title">👥 Grupos VIP</div>
      </div>
    ...
    <div id="section-users" class="section">
      <div class="filters">
        <input class="filter-input" id="userSearch" placeholder="🔍 Nome, username ou Telegram ID..." oninput="loadUsers()">
    ...
    <div id="section-broadcast" class="section">
      <div class="section-header">
        <div class="section-title">📢 CastCupom — Histórico de Campanhas</div>
    ...
    <div id="section-settings" class="section">
      <div class="section-title" style="margin-bottom:24px">⚙️ Configurações</div>
      <div class="pix-card">
        <div style="font-size:16px;font-weight:700;margin-bottom:6px">🔑 Chave PIX</div>
    ...
    <div id="section-stats" class="section">
      <div class="section-title" style="margin-bottom:24px">📈 Estatísticas</div>
      <div class="dash-grid">
        <div class="dash-card">
          <div class="dash-card-title">📅 Vendas por dia (30d)</div>
          <div id="chartByDay" style="font-size:12px;color:var(--text2)"></div>
        </div>
        <div class="dash-card">
          <div class="dash-card-title">📊 Transações por status</div>
...
<div class="modal-overlay" id="modalProduct">
  <div class="modal">
    ...
    <div class="form-row"><label>Tipo de Entrega</label>
      <select id="p_type"><option value="link">Link</option><option value="media_pack">Pack de Mídia</option><option value="group">Grupo</option></select>
    </div>
...
<div class="modal-overlay" id="modalTicket">
  <div class="modal" style="max-width:560px">
    ...
    <div class="modal-footer">
      ...
      <button class="btn btn-danger btn-sm" id="btnCloseTicket">✓ Resolver Ticket</button>
    </div>
  </div>
</div>
...
// ===== NAVIGATION =====
const sectionTitles = {
  dashboard: 'Dashboard', transactions: 'Transações', failures: 'Falhas de Entrega',
  products: 'Produtos', groups: 'Grupos VIP', users: 'Usuários',
  tickets: 'Tickets de Suporte', broadcast: 'CastCupom', settings: 'Configurações', stats: 'Estatísticas'
};
...
async function loadDashboard() {
  ...
  document.getElementById('dashStats').innerHTML = \`
    <div class="stat-card accent"><div class="stat-icon">💰</div><div class="stat-value">R$ \${fmt(d.totalSales)}</div><div class="stat-label">Receita Total</div></div>
    <div class="stat-card success"><div class="stat-icon">📈</div><div class="stat-value">R$ \${fmt(d.vendasHoje)}</div><div class="stat-label">Vendas Hoje</div></div>
    <div class="stat-card info"><div class="stat-icon">📅</div><div class="stat-value">R$ \${fmt(d.vendasSemana)}</div><div class="stat-label">Esta Semana</div></div>
    <div class="stat-card warning"><div class="stat-icon">⏳</div><div class="stat-value">\${d.pendentes}</div><div class="stat-label">Pendentes</div></div>
    <div class="stat-card"><div class="stat-icon">👥</div><div class="stat-value">\${(d.users||0).toLocaleString()}</div><div class="stat-label">Usuários</div></div>
    <div class="stat-card"><div class="stat-icon">💳</div><div class="stat-value">\${(d.transactions||0).toLocaleString()}</div><div class="stat-label">Transações</div></div>
    <div class="stat-card"><div class="stat-icon">🎫</div><div class="stat-value">\${d.tickets}</div><div class="stat-label">Tickets Abertos</div></div>
    <div class="stat-card"><div class="stat-icon">📢</div><div class="stat-value">\${d.broadcastAtivo}</div><div class="stat-label">Broadcasts Ativos</div></div>
  \`;
  ...
  const txHtml = d.recentTx?.map(t => \`
    ...
    </div>\`).join('') || '<div class="empty-state" style="padding:20px">Nenhuma transação</div>';
  ...
  const usersHtml = d.recentUsers?.map(u => \`
    ...
    </div>\`).join('') || '<div class="empty-state" style="padding:20px">Nenhum usuário</div>';
...
async function loadTransactions() {
  ...
  const rows = d.data.map(t => \`
    <tr>
      ...
      <td>
        \${t.status === 'pending' ? \`<button class="btn btn-success btn-sm" onclick="approveTransaction('\${t.txid}')">✓ Aprovar</button>
        <button class="btn btn-danger btn-sm" onclick="rejectTransaction('\${t.txid}')">✗ Rejeitar</button>\` : '—'}
      </td>
    </tr>\`).join('');

  document.getElementById('txTable').innerHTML = \`
    <table>
      <thead><tr><th>TXID</th><th>Telegram ID</th><th>Valor</th><th>Status</th><th>Data</th><th>Ações</th></tr></thead>
      <tbody>\${rows || '<tr><td colspan="6" style="text-align:center;padding:40px;color:var(--muted)">Nenhuma transação encontrada</td></tr>'}</tbody>
    </table>\`;
...
async function loadFailures() {
  ...
  if (!d.data?.length) {
    document.getElementById('failuresTable').innerHTML = '<div class="empty-state"><div class="empty-icon">✅</div><div>Nenhuma falha de entrega!</div></div>';
    return;
  }
  ...
  document.getElementById('failuresTable').innerHTML = \`
    <table>
      <thead><tr><th>TXID</th><th>Telegram ID</th><th>Valor</th><th>Tipo Erro</th><th>Tentativas</th><th>Última Tentativa</th></tr></thead>
      <tbody>\${rows}</tbody>
    </table>\`;
}
...
async function loadProducts() {
  ...
  if (!d.data?.length) {
    document.getElementById('productsTable').innerHTML = '<div class="empty-state"><div class="empty-icon">🛍️</div><div>Nenhum produto cadastrado</div></div>';
    return;
  }
  ...
  document.getElementById('productsTable').innerHTML = \`
    <table>
      <thead><tr><th>Nome</th><th>ID</th><th>Preço</th><th>Tipo</th><th>Status</th><th>Criado</th></tr></thead>
      <tbody>\${rows}</tbody>
    </table>\`;
}
...
async function saveProduct() {
  ...
  if (d.ok) { toast('✅ Produto criado!', 'success'); closeModal('modalProduct'); loadProducts(); }
...
async function loadGroups() {
  ...
  if (!d.data?.length) {
    document.getElementById('groupsTable').innerHTML = '<div class="empty-state"><div class="empty-icon">👥</div><div>Nenhum grupo cadastrado</div></div>';
    return;
  }
  ...
  document.getElementById('groupsTable').innerHTML = \`
    <table>
      <thead><tr><th>Nome</th><th>ID</th><th>Preço</th><th>Duração</th><th>Status</th><th>Ações</th></tr></thead>
      <tbody>\${rows}</tbody>
    </table>\`;
}
...
async function toggleGroup(id, active) {
  ...
  toast(active ? '✅ Grupo ativado!' : '⏸ Grupo pausado.', 'success');
  loadGroups();
}
...
async function loadUsers() {
  ...
  document.getElementById('usersTable').innerHTML = \`
    <table>
      <thead><tr><th>Nome</th><th>Username</th><th>Telegram ID</th><th>Status</th><th>Cadastro</th><th>Ações</th></tr></thead>
      <tbody>\${rows || '<tr><td colspan="6" style="text-align:center;padding:40px;color:var(--muted)">Nenhum usuário encontrado</td></tr>'}</tbody>
    </table>\`;
...
async function toggleBlock(telegram_id, block) {
  ...
  toast(block ? '🔒 Usuário bloqueado.' : '🔓 Usuário desbloqueado!', block ? 'error' : 'success');
  loadUsers();
}
...
async function loadTickets() {
  ...
  const rows = d.data.map(t => \`
    <tr>
      ...
      <td>
        <button class="btn btn-ghost btn-sm" onclick="viewTicket('\${t.id}', '\${t.subject || 'Ticket'}', '\${t.status}')">👁 Ver</button>
        \${t.status !== 'resolved' ? \`<button class="btn btn-success btn-sm" onclick="closeTicket('\${t.id}')">✓ Resolver</button>\` : ''}
      </td>
    </tr>\`).join('');

  document.getElementById('ticketsTable').innerHTML = \`
    <table>
      <thead><tr><th>#</th><th>Usuário</th><th>Assunto</th><th>Status</th><th>Data</th><th>Ações</th></tr></thead>
...
async function viewTicket(id, subject, status) {
  ...
  const msgs = d.data?.map(m => \`
    <div class="ticket-msg \${m.is_admin ? 'admin' : 'user'}">
      <div>\${m.message}</div>
      <div class="ticket-msg-meta">\${m.is_admin ? '👑 Admin' : '👤 Usuário'} • \${fmtDate(m.created_at)}</div>
...
async function loadBroadcast() {
  ...
  if (!d.data?.length) {
    document.getElementById('broadcastList').innerHTML = '<div class="empty-state"><div class="empty-icon">📢</div><div>Nenhuma campanha criada ainda</div></div>';
    return;
  }
  document.getElementById('broadcastList').innerHTML = d.data.map(c => {
    ...
    <div class="broadcast-card">
      <div style="font-size:32px">\${c.status === 'sent' ? '✅' : c.status === 'sending' || c.status === 'pending' ? '📡' : '⚠️'}</div>
      <div class="broadcast-progress">
        ...
        <div style="font-size:12px;color:var(--text2);margin-top:4px">
          ✅ \${c.success_count||0} enviados &nbsp; ❌ \${c.failed_count||0} falhas &nbsp; 👥 \${c.total_users||'?'} total
        </div>
        <div class="progress-bar"><div class="progress-fill" style="width:\${pct}%"></div></div>
        <div style="font-size:11px;color:var(--muted);margin-top:4px">\${fmtDate(c.created_at)} &nbsp;•&nbsp; \${pct}% concluído</div>
      </div>
    </div>\`;
  }).join('');
}
...
async function savePixKey() {
  ...
  if (!pixKey) return toast('❌ Chave PIX não pode ser vazia', 'error');
...
async function loadStats() {
  ...
  // Vendas por dia — mini chart de barras em CSS
  const byDay = d.byDay || {};
...
function badge(status) {
  const map = { pending:'badge-pending pending', approved:'badge-approved approved', delivered:'badge-delivered delivered', rejected:'badge-rejected rejected', delivery_failed:'badge-failed failed', sending:'badge-sending enviando', pending_broadcast:'badge-sending pendente', sent:'badge-delivered concluído', open:'badge-open aberto', resolved:'badge-resolved resolvido', closed:'badge-resolved fechado', unknown:'badge-failed desconhecido', blocked:'badge-blocked bloqueado', temporary:'badge-warning temporário' };
...
function renderPagination(containerId, current, total, cb) {
  ...
  let html = \`<button class="page-btn" onclick="(\${cb.toString()})(\${Math.max(1,current-1)})" \${current<=1?'disabled':''}>←</button>\`;
  for (let i = Math.max(1,current-2); i <= Math.min(total,current+2); i++) {
    html += \`<button class="page-btn \${i===current?'active':''}" onclick="(\${cb.toString()})(\${i})">\${i}</button>\`;
  }
  html += \`<button class="page-btn" onclick="(\${cb.toString()})(\${Math.min(total,current+1)})" \${current>=total?'disabled':''}>→</button>\`;
  html += \`<span class="page-info">Pág \${current}/\${total}</span>\`;

...
@media(max-width:768px){
  ...
  .content{padding:12px;height:calc(100vh - 48px - var(--mnav));overflow-x:hidden;max-width:100vw}
  .btnhb{display:flex}
  ...
  .dg{grid-template-columns:1fr;width:100%;max-width:100%}
  /* hide table cols */
...
/* ════════════════════════════════════════════════════════
   MOBILE PREMIUM — Responsividade profissional
   Compatível 320px → 768px | Desktop intacto
════════════════════════════════════════════════════════ */

/* ── iOS: previne zoom automático em inputs ─────────── */
@media(max-width:768px){
...
@media(max-width:768px){
  ...
  .btnhb{display:none !important}      /* esconde hamburger */
}
...
@media(max-width:768px){
  ...
  .tb-brand-m svg{width:20px;height:20px;stroke:var(--violl);fill:none;stroke-width:2}
  .tb-brand-m span{
    font-size:15px;font-weight:700;letter-spacing:-.3px;
    color:var(--fg)
  }
  .tb-brand-m em{color:var(--violl);font-style:normal}
  .crumb-c{font-size:14px;font-weight:600;letter-spacing:-.2px;color:var(--fg3)}
}
@media(min-width:769px){
  .tb-brand-m{display:none !important}
}

/* ── SIDEBAR — off-canvas premium ──────────────────── */
@media(max-width:768px){
  .sb{
    width:min(var(--sb,280px),85vw);
    will-change:transform;
    transition:transform .28s cubic-bezier(.16,1,.3,1),
               box-shadow .28s ease
  }
  .sb.on{box-shadow:12px 0 60px rgba(0,0,0,.7)}
  .sb-ov{
    backdrop-filter:blur(2px);
    -webkit-backdrop-filter:blur(2px);
    transition:opacity .25s
  }
  .sb-top{height:54px;padding:0 16px}
  .ni{padding:10px 12px;min-height:44px;border-radius:8px;font-size:13.5px}
  .nico{font-size:14px}
}

/* ── CONTENT AREA — remove offset da sidebar no mobile ── */
@media(max-width:768px){
  .main{margin-left:0 !important}  /* sidebar oculta, sem offset */
  .content{
    padding:14px 12px calc(var(--mnav,68px) + env(safe-area-inset-bottom,0px) + 20px);
    overscroll-behavior-y:contain;
    height:calc(100vh - 52px - var(--mnav,68px) - env(safe-area-inset-bottom,0px))
  }
}

/* ── BOTTOM NAV — safe area + glassmorphism ─────────── */
@media(max-width:768px){
  .mnav{
    height:auto;
    padding:6px 2px max(12px,env(safe-area-inset-bottom,12px));
    backdrop-filter:blur(18px);
    -webkit-backdrop-filter:blur(18px);
    background:rgba(6,6,8,.92);
    border-top:1px solid var(--line2)
  }
  .mni{padding:5px 6px;min-width:52px;gap:3px}
  .mni-ico{width:36px;height:36px;border-radius:10px}
  .mni span{font-size:10px;font-weight:500}
}

/* ── DRAWER ─────────────────────────────────────────── */
@media(max-width:768px){
  .drawer{
    bottom:calc(var(--mnav,68px) + env(safe-area-inset-bottom,0px) + 10px);
    border-radius:20px;padding:14px;max-height:76vh
  }
  .dgrid{gap:8px}
  .ditem{padding:14px 6px;border-radius:13px;font-size:11px}
  .dico{font-size:22px}
}
...
@media(max-width:768px){
  .toasts{
    bottom:calc(var(--mnav,68px) + env(safe-area-inset-bottom,0px) + 14px);
...
@media(max-width:768px){
  ...
  .modal{
    ...
    padding:0 16px max(28px,env(safe-area-inset-bottom,28px)) !important
  }
  .modal::before{
    content:'';display:block;
    width:44px;height:5px;
    background:var(--line3);border-radius:3px;
    margin:10px auto 18px
  }
  .mtitle{font-size:16px;font-weight:600;margin-bottom:16px}
  .mfoot{
    flex-direction:column-reverse !important;
    gap:8px;padding-top:14px;margin-top:16px
  }
  .mfoot .btn{
...
@media(max-width:768px){
  .btn{min-height:40px}
  .btn.bfl{min-height:52px;border-radius:14px;font-size:15px}
  .fi{height:48px !important;padding:10px 14px !important}
  .frow .fi{height:48px !important}
  .frow textarea.fi{height:auto !important;min-height:100px}
  .frow{margin-bottom:16px}
  .frow label{font-size:11px;margin-bottom:6px}
  .filters{gap:6px}
  .filters .fi{height:42px !important}
}
...
@media(max-width:768px){
  ...
  .kpis-m{gap:10px}
  .kpis-m .kpi{padding:16px;border-radius:14px}
  .kpis-m .kpi-v{font-size:26px;letter-spacing:-1.5px}
  .ustrip{gap:10px}
  .uc{border-radius:14px;padding:16px}
  .uc-val{font-size:30px;letter-spacing:-1.5px}
  .qab{min-height:40px;padding:10px 16px;border-radius:24px;font-size:13px}
  .dg{grid-template-columns:1fr;gap:10px;margin-top:12px}
  .dli{padding:12px 16px}
}

/* ── TABELAS — scroll horizontal elegante ───────────── */
@media(max-width:768px){
  .tbl-w{
    overflow-x:auto;
  ...
  thead th{padding:7px 10px;font-size:10px;white-space:nowrap}
  td{padding:9px 10px;font-size:12.5px}
}

/* ── PAGER ──────────────────────────────────────────── */
@media(max-width:768px){
  .pager{flex-wrap:wrap;gap:5px;margin-top:14px;justify-content:center}
  .pg{min-height:38px;min-width:38px;border-radius:8px}
}

/* ── BROADCAST cards ────────────────────────────────── */
@media(max-width:768px){
  .bc{border-radius:14px;padding:14px;gap:10px;margin-bottom:10px}
  .bc-body{width:100%}
  .bc-body>div:last-child{display:flex;flex-wrap:wrap;gap:6px;margin-top:6px}
  .bc-body>div:last-child .btn{flex:1;min-width:120px;justify-content:center;min-height:42px}
}

/* ── SETTINGS ───────────────────────────────────────── */
@media(max-width:768px){
  .sblock{padding:16px;border-radius:14px;margin-bottom:12px}
  .sblock h3{font-size:14px}
  .sblock p{font-size:12.5px;margin-bottom:14px}
}

/* ── MEDIA GRID ─────────────────────────────────────── */
@media(max-width:768px){
  .mgrid{grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:6px}
  .mthumb{border-radius:9px}
}

/* ── USER DETAIL ────────────────────────────────────── */
@media(max-width:768px){
  .udh{flex-wrap:wrap;gap:12px}
  .udg{grid-template-columns:1fr 1fr;gap:8px}
  .udgi{padding:12px}
  .udgi-v{font-size:22px}
}

/* ── THREAD ─────────────────────────────────────────── */
@media(max-width:768px){
  .thread{max-height:240px}
  .msg{font-size:13px;border-radius:10px;padding:8px 12px}
}

/* ── TRUST BAR ──────────────────────────────────────── */
.tbar{height:6px;border-radius:3px;background:var(--s3);overflow:hidden}
.tfill{height:100%;border-radius:3px;transition:width .4s ease}

/* ── REFINAMENTOS 390px ─────────────────────────────── */
@media(max-width:390px){
  :root{--sb:272px}
  .mni{min-width:46px;padding:5px 4px}
  .mni span{font-size:9.5px}
  .kpis-m .kpi{min-width:136px}
  .card-h{padding:12px 14px 10px}
}

/* ── EXTREME SMALL 320px ────────────────────────────── */
@media(max-width:320px){
  :root{--sb:256px}
  .mni-ico{width:30px;height:30px;font-size:15px}
  .kpis-m .kpi{padding:12px}
  .ditem{font-size:10px;padding:10px 3px}
  .dico{font-size:20px}
}

/* ── DASHBOARD — novos usuários cards ──────────────── */
.t-creator{background:var(--ok-bg,rgba(16,185,129,.18));color:#10b981}
.t-v{background:var(--violl-bg,rgba(139,92,246,.18));color:var(--violl)}
.t-g{background:rgba(16,185,129,.18);color:#10b981}
#dUsers>div:last-child{border-bottom:none !important}
#dUsers>div:hover{background:var(--s3)}
</style>
</head>
<body>
<div class="wrap">
...
<div class="main" id="main">
  <div class="tb">
    <div style="display:flex;align-items:center;gap:10px">
      ...
      <!-- Brand logo: só aparece no mobile (sidebar oculta) -->
      <a class="tb-brand-m" onclick="go('dashboard',document.querySelector('.ni[onclick*=dashboard]'))" style="cursor:pointer">
        <svg viewBox="0 0 24 24"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/></svg>
        <span>Nexus <em>Panel</em></span>
      </a>
...
function toggleSB(){
  ...
  // Trava scroll do body no mobile quando sidebar aberta
...
function go(sec,el){
  ...
  cur=sec;
  // ✅ FIX: salvar aba atual na URL hash para persistir no refresh
  try{location.hash=sec;}catch(_){}
  if(isMob())closeSB();
...
async function loadDash(){
  ...
  // Recent users — cards ricos
  document.getElementById('dUsers').innerHTML=(d.recentUsers||[]).map(function(u){
    var nm=(u.first_name||'?');
    var ini=nm.charAt(0).toUpperCase();
    var tgLink=u.username?'https://t.me/'+u.username:'';
    var badge=u.is_blocked?'<span class="tag t-blocked" style="font-size:9px;margin-left:4px">bloq</span>'
             :u.is_admin?'<span class="tag t-v" style="font-size:9px;margin-left:4px">admin</span>'
             :u.is_creator?'<span class="tag t-g" style="font-size:9px;margin-left:4px">creator</span>':'';
    var phone=u.phone_number?'<span style="font-size:10px;color:var(--fg3)">📞 '+u.phone_number+'</span>':'';
    var uname=u.username?'<a href="'+tgLink+'" target="_blank" class="mono" style="font-size:10.5px;color:var(--violl)" onclick="event.stopPropagation()">@'+u.username+'</a>':'';
    var tid='<span class="mono" style="font-size:10px;color:var(--fg3)">'+u.telegram_id+'</span>';
    return '<div style="display:flex;align-items:center;gap:10px;padding:11px 16px;border-bottom:1px solid var(--line2);cursor:pointer" onclick="viewUser('+u.telegram_id+')">'  
      +'<div style="width:36px;height:36px;border-radius:10px;background:var(--s3);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:15px;color:var(--violl);flex-shrink:0">'+ini+'</div>'
      +'<div style="flex:1;min-width:0;overflow:hidden">'
        +'<div style="font-weight:600;font-size:13px;display:flex;align-items:center">'+nm+badge+'</div>'
        +'<div style="display:flex;gap:8px;margin-top:3px;flex-wrap:wrap;align-items:center">'+uname+tid+phone+'</div>'
      +'</div>'
      +'<div style="text-align:right;flex-shrink:0;font-size:9.5px;color:var(--fg3);font-family:monospace">'+fmtDs(u.created_at)+'</div>'
    +'</div>';
...
// ── INIT ─────────────────────────────────────────────
// ── iOS Viewport Height fix (Safari) ──────────────────
(function setVH(){
...
// ── Swipe gesture: deslize da borda esquerda → abre sidebar
// ── Deslize esquerda com sidebar aberta → fecha
(function(){
  var sx=0,sy=0,st=0;
  ...
  document.addEventListener('touchend',function(e){
    ...
    var dt=Date.now()-st;
    if(Math.abs(dy)>Math.abs(dx)*1.4)return; // scroll vertical
    if(dt>500)return;                          // muito lento
    var sb=document.getElementById('sb');
    if(!sb)return;
    // Swipe right da borda → abre
    if(dx>55&&sx<36&&!sb.classList.contains('on')){toggleSB();return;}
    // Swipe left → fecha
    if(dx<-55&&sb.classList.contains('on')){closeSB();}
  },{passive:true});
})();

checkAuth().then(()=>{
  // ✅ FIX: restaurar aba salva no hash (persiste refresh)
  const hash=(location.hash||'').replace('#','').trim();
  const validSecs=Object.keys(loads);
  if(hash&&validSecs.includes(hash)){

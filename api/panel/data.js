  ...
  try {
    ...
    if (action === 'dashboard') {
      ...
      const [users, txCount, pendentes, tickets, failures, totalSalesR, hojeR, semanaR, avgR, recentTx, recentUsers, trustedR] = await Promise.all([
        ...
        supabase.from('users').select('telegram_id, first_name, last_name, username, created_at, phone_number, is_blocked, is_admin, is_creator').not('telegram_id', 'is', null).order('created_at', { ascending: false }).limit(10),

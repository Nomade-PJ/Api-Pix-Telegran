...
${zwsp}${zwnj}${zwsp}`, {
        ...
        try {
          ...
          if (errorType === 'blocked') {
            // ✅ CORREÇÃO ERRO 2: Usuário bloqueou o bot — NÃO marcar como delivered
            deliveryBlocked = true;
            console.warn(`🚫 [ADMIN-APPROVE] Usuário ${transaction.telegram_id} bloqueou o bot — marcando como delivery_failed`);
            await db.supabase
              .from('transactions')
              .update({
                status: 'delivery_failed',
                delivery_error_type: 'blocked',
            ...
            await ctx.editMessageReplyMarkup({
              inline_keyboard: [[{ text: '🚫 Bloqueado — entrega falhou', callback_data: 'blocked' }]]
            });
            return ctx.reply(
              `⚠️ *Aprovado, mas entrega falhou*\n\n` +
              `🆔 TXID: ${txid}\n` +
              `🚫 Motivo: Usuário bloqueou o bot\n\n` +
              `Transação marcada como *delivery_failed*.\n` +
              `O usuário deve desbloquear o bot e enviar /start.`,
              { parse_mode: 'Markdown' }
            );
          }

          // Para outros erros: avisar o usuário
          try {

...
const { selectPixKey, logPixUsage } = require('./selectPixKey');
...
async function createManualCharge({ amount = "10.00", productId, userId = null }) {
  try {
    // ── Decidir qual chave PIX usar (principal ou secundária) ──────────────
    const pixDecision = await selectPixKey(userId, amount);
    const rawKey = pixDecision.pix_key || await db.getPixKey();
    ...
    try {
      ...
      console.log(`✅ [PIX] Chave ${pixDecision.pix_type} sanitizada: "${rawKey}" -> "${key}" | Motivo: ${pixDecision.reason}`);
    } catch (sanitizeError) {
    ...
    // Registrar log de qual chave foi usada (não bloqueia o fluxo)
    logPixUsage(txid, userId, amountFormatted, key, pixDecision.pix_type, pixDecision.reason)
      .catch(() => {});
    ...
    return {
      ...
      charge: {
        ...
        pixType: pixDecision.pix_type,   // 'primary' ou 'secondary'
        pixReason: pixDecision.reason,   // motivo da escolha

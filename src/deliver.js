...
// Salva o message_id de cada envio para permitir deleção futura
// via script deletar_midias.js
// ============================================================

async function saveMessageSent({ telegramId, userId, messageId, transactionId, packId, productTag, mediaType, fileUrl }) {
  try {
    ...
    console.error('⚠️ [MSG-SENT] Erro ao salvar message_id:', err.message);
  }
}
...
async function deliverProductFromStorage(chatId, productId, productName, opts = {}) {
  ...
  // Mensagem inicial
  const msgInicial = await withDeliveryRetry(() =>
    tg.sendMessage(chatId,
      `✅ *PAGAMENTO CONFIRMADO!*\n\n📦 *${productName}*\n\n🚀 Enviando *${mediaFiles.length}* ${mediaFiles.length > 1 ? 'arquivos' : 'arquivo'}...`,
      { parse_mode: 'Markdown' }
    )
  );
  await saveMessageSent({ telegramId: chatId, userId, messageId: msgInicial.message_id, transactionId, productTag: productId, mediaType: 'text' });

  // Montar URLs públicas
  const mediaWithUrls = mediaFiles.map((file, idx) => ({
    type:       /\.(mp4|mov|avi|mkv|webm)$/i.test(file.name) ? 'video' : 'photo',
    media:      `${SUPABASE_URL}/storage/v1/object/public/media-packs/${folder}/${file.name}`,
    caption:    idx === 0 ? `✨ *${productName}*` : undefined,
    parse_mode: idx === 0 ? 'Markdown' : undefined,
    _name:      file.name,  // interno
  }));
  ...
  for (let i = 0; i < mediaWithUrls.length; i += BATCH_SIZE) {
    batchNum++;
    const batchOriginal = mediaWithUrls.slice(i, i + BATCH_SIZE);
    // Remover campo interno antes de enviar ao Telegram
    const batchTelegram = batchOriginal.map(({ _name, ...item }) => item);

    try {
      console.log(`📤 [DELIVER-STORAGE] Enviando lote ${batchNum} (${batchTelegram.length} itens)...`);

      // sendMediaGroup retorna array — cada item tem message_id próprio
      const sentList = await withDeliveryRetry(() =>
        tg.sendMediaGroup(chatId, batchTelegram)
      );

      // Rastrear cada mensagem do lote individualmente
      for (let j = 0; j < sentList.length; j++) {
        await saveMessageSent({
          telegramId:    chatId,
          userId,
          messageId:     sentList[j].message_id,
          transactionId,
          productTag:    productId,
          packId:        folder,
          mediaType:     batchOriginal[j].type,
          fileUrl:       batchOriginal[j].media
        });
      }

      sentCount += batchTelegram.length;
      ...
      // Fallback: enviar arquivo por arquivo
      for (const item of batchOriginal) {
        try {
          let sentSingle;
          if (item.type === 'video') {
            sentSingle = await withDeliveryRetry(() =>
          ...
          await saveMessageSent({
            telegramId: chatId, userId,
            messageId:  sentSingle.message_id,
            transactionId,
            productTag: productId,
            packId:     folder,
            mediaType:  item.type,
  ...
  // Mensagem final
  const msgFinal = await withDeliveryRetry(() =>
    tg.sendMessage(chatId,
      `🎉 *Entrega completa!*\n\n✅ ${sentCount} ${sentCount > 1 ? 'arquivos enviados' : 'arquivo enviado'} com sucesso!\n\nObrigado pela preferência! 💚`,
      { parse_mode: 'Markdown' }
    )
  );
  await saveMessageSent({ telegramId: chatId, userId, messageId: msgFinal.message_id, transactionId, productTag: productId, mediaType: 'text' });

  return { success: true, sent: sentCount, total: mediaWithUrls.length };
}

// ============================================================
// FUNÇÕES DE ENTREGA EXISTENTES (sem alteração na lógica)
// ============================================================
...
async function deliverMediaPack(chatId, packId, userId, transactionId, db) {
  try {
    ...
    // Confirmação (rastreada)
    const msgConfirm = await withDeliveryRetry(() =>
    ...
    // Conclusão (rastreada)
    const msgFim = await withDeliveryRetry(() =>
    ...
    await saveMessageSent({ telegramId: chatId, userId, messageId: msgFim.message_id, transactionId, packId, mediaType: 'text' });
